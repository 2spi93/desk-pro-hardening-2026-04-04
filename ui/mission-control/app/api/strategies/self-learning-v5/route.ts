...
import {
  parseSelfLearningV5Scope,
  readSelfLearningV5State,
  writeSelfLearningV5State,
} from "../../../../lib/selfLearningV5Store";
...
const selfLearningGlobal = globalThis as typeof globalThis & {
  __mcSelfLearningV5ReadCache?: Map<string, ReadCacheEntry>;
};

const readCache = selfLearningGlobal.__mcSelfLearningV5ReadCache || new Map<string, ReadCacheEntry>();
if (!selfLearningGlobal.__mcSelfLearningV5ReadCache) {
  selfLearningGlobal.__mcSelfLearningV5ReadCache = readCache;
}
...
export async function GET(request: Request) {
  ...
  try {
    const { response: cpResponse, payload } = await cpFetchJsonSafe(
      `/v1/strategies/self-learning-v5?account_id=${encodeURIComponent(scope.accountId)}&symbol=${encodeURIComponent(scope.symbol)}&timeframe=${encodeURIComponent(scope.timeframe)}`,
      { method: "GET" },
    );
    if (cpResponse.ok) {
    ...
    if (cpResponse.status === 401 || cpResponse.status === 403) {
      const state = await readSelfLearningV5State(scope).catch(() => null);
      const normalized = {
        status: "ok",
        state,
        ...
        degraded: true,
        detail: "self_learning_v5_anonymous_degraded",
    ...
    const state = await readSelfLearningV5State(scope);
    ...
    const normalized = {
      ...
      storage: "local-fallback",
      degraded: true,
      detail: "self_learning_v5_unreachable",
    };
    readCache.set(cacheKey, { atMs: nowMs, payload: normalized });
    return noStoreJson(normalized, 200);
  }
...
export async function PUT(request: Request) {
  ...
  try {
    const { response: cpResponse, payload } = await cpFetchJsonSafe("/v1/strategies/self-learning-v5", {
      method: "PUT",
      headers: {
    ...
    const state = await writeSelfLearningV5State(body);
    return noStoreJson({ status: "ok", state, updatedAt: state.updatedAt, storage: "local-fallback" }, 200);
  } catch (error) {
    if (error instanceof Error && error.message === "invalid_self_learning_v5_state") {
      return noStoreJson({ status: "error", message: "invalid self-learning v5 state" }, 400);
    }
    return noStoreJson(
      {
        status: "ok",
        ...
        storage: "local-fallback",
        degraded: true,
        detail: "self_learning_v5_persist_unreachable",
      },
      200,
    );
  }
