...
import { listSelfLearningV5Scopes } from "../../../../../lib/selfLearningV5Store";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const { response: cpResponse, payload } = await cpFetchJsonSafe(`/v1/strategies/self-learning-v5/scopes?${params.toString()}`, { method: "GET" });
    if (cpResponse.ok) {
    ...
    if (cpResponse.status === 401 || cpResponse.status === 403) {
      const items = await listSelfLearningV5Scopes({ accountId, symbol, timeframe, limit }).catch(() => []);
      return noStoreJson({
        status: "ok",
        items,
        total: items.length,
        ...
        degraded: true,
        detail: "self_learning_v5_scopes_anonymous_degraded",
        upstream_status: cpResponse.status,
      }, 200);
    }
    const items = await listSelfLearningV5Scopes({ accountId, symbol, timeframe, limit });
    ...
    return noStoreJson({
      ...
      storage: "local-fallback",
      degraded: true,
      detail: "self_learning_v5_scopes_unreachable",
    }, 200);
  }
}